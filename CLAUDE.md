...
6. **设计原则**：代码设计需符合 SOLID 原则（单一职责、开放封闭、里氏替换、接口隔离、依赖倒置）
7. **路径限制**：所有文件必须在项目路径内创建
8. **阶段前置**：执行每阶段前，必须先阅读 `.analysis/` 下所有文档
9. **自我检验**：每阶段完成后，先自检（准确性、格式、链接）再提交审核
10. **代码质量**：示例代码必须通过编译和测试；UT 覆盖率要求 100%，即便测试通过也必须达到 100%
11. **提交流程**：用户确认通过 → git commit → 进入下一阶段
12. **防止幻觉**：每一阶段  用户确认之后，进入下一阶段之前，清空当前窗口的 context，再进入下一阶段。
